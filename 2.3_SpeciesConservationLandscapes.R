...
##########################
WS <- "E:/Ellie/BigCats/"
intersection.stats.outWS <- str_c(WS, "AnalysisOutputs_2026_June/SpeciesConservationLandscapes/version_3_0/IntersectionStats/")

#####################
### Set Variables ###
#####################
scl_category_order <- read.csv(str_c(WS, "Data/SpeciesConservationLandscapes/scl_category_order_20260609.csv"))
...
scl.outWS <- str_c(WS, "AnalysisOutputs_2026_June/SpeciesConservationLandscapes/version_3_0/")
...
for (species in species_folders) {
  ...
  for (file in geojson_files) {
    ...
    sf_object <- st_read(file, quiet = TRUE) %>%
      ...
      left_join(Country.Code.Lookup, by = c("iso2" = "ISO2")) %>%
...
boundary_layers <- list(KBAs = KBAs %>% st_make_valid(),
                        ProtectedAreas = ProtectedAreas %>% st_make_valid())
...
repair.geometry.validity.outWS <- str_c(WS, "AnalysisOutputs_2026_June/SpeciesConservationLandscapes/version_3_0/RepairGeometryValidity/")
...
### Export tables ###
# write.table(results_KBAs, str_c(intersection.stats.outWS, "SCL_x_KBAs_20260618.csv"), row.names = FALSE, col.names = TRUE)
# write.table(results_ProtectedAreas, str_c(intersection.stats.outWS, "SCL_x_ProtectedAreas_20260618.csv"), row.names = FALSE, col.names = TRUE)
