...
ProtectedAreas <- st_read(dsn = str_c(WS,"AnalysisOutputs/ProtectedAreas/Merged/ProtectedAreas_dissolved_merged_all.gdb"),
...
Country.ISO3.codes <- Country.ISO3.codes %>% 
  select(Country.or.Area, ISO2, ISO3) %>% 
...
gbif.clean.big.cats <- gbif.clean.big.cats %>%
  ...
  left_join(Country.ISO3.codes, by = c("countryCode" = "ISO2")) %>%
...
for (i in seq_along(boundary_layers)) {
  ...
  results_list[[i]] <- points_within %>%
    ...
    left_join(Country.ISO3.codes %>% select(ISO3, CountryName), by = "ISO3") %>%
...
# st_write(gbif.clean.big.cats.sf.WebMercator, str_c(WS, "AnalysisOutputs/GBIF/Felidae_download_20260427/GBIF_Spatial_WebMercator/GBIF_BigCats_20260514.shp"))
...
# write.table(summary_df, str_c(WS, "AnalysisOutputs/GBIF/Felidae_download_20260427/SummaryStats/GBIF_summary_20260519.csv"), row.names = FALSE, col.names = TRUE)
print("Data exported")
